...
M.register = function(bufnr)
    ...
    map("n", "[h", "&diff ? '[c' : ':Gitsigns prev_hunk<CR>'", { expr = true, desc = "Previous hunk" })

    -- Actions
    map("n", "<leader>hb", function()
        require("gitsigns").blame_line({ full = true })
    end, {
        desc = "Show git blame per line",
    })
    ...
    map("n", "<leader>tb", ":Gitsigns toggle_current_line_blame<CR>", { desc = "Toggle current line blame" })
